...
class Game:
    def __init__(self, window: Window, asset_handler: AssetHandler, mods: list[Mod]) -> None:
        self.window = window 
        ...
        self.story_mode_main_menu_fps: int = 16
        ...
        self.freeplay_main_menu_dt_accumulator: float = 0.0
        self.freeplay_main_menu_fps: int = 16
        self.freeplay_main_menu_frame_interval: int = 1.0 / self.freeplay_main_menu_fps
        self.freeplay_main_menu_curr_frame: int = 0

        self.freeplay_main_menu_text_visible: bool = False
        self.freeplay_main_menu_clicked: bool = False

        self.main_menu_can_switch: bool = True
        self.main_menu_choice: int = 0
        self.main_menu_last_possible_choice: int = 1
    ...
    def update(self, dt: int, ev_handler: "EventHandler") -> None:
        ...
        if self.state == "menu":
            if self.main_menu_choice == 0:
                self.freeplay_main_menu_text_visible = True

                self.story_mode_main_menu_text_visible = False
            else:
                self.story_mode_main_menu_text_visible = True

                self.freeplay_main_menu_text_visible = False
            ...
            self.freeplay_main_menu_dt_accumulator += dt 

            if self.freeplay_main_menu_dt_accumulator >= self.freeplay_main_menu_frame_interval:
                self.freeplay_main_menu_dt_accumulator -= self.freeplay_main_menu_frame_interval
                self.freeplay_main_menu_curr_frame += 1

                if self.freeplay_main_menu_clicked:
                    self.freeplay_main_menu_text_visible = not self.freeplay_main_menu_text_visible

            if self.freeplay_main_menu_curr_frame >= 3:
                self.freeplay_main_menu_curr_frame = 0
    ...
    def render(self) -> None:
        ...
        if self.state == "freeplay":
            self.window.blit(self.asset_handler.bg, (0, 0))
            self.mods_len = len(self.mods)

            for i in range(0, 5): # display only 5 mods for now, since no scrolling.
                if i >= self.mods_len: break

                mod = self.mods[i]

                self.window.blit(
                    self.asset_handler.giant_font.render(mod.mod_name, True, (255, 255, 255)),
                    (self.window.width // 2 - 800, self.window.height - (1000 - 200 * i))    
                )
